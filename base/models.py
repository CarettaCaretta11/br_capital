...
class Investor(AbstractUser):
    ...
    def invest(self, startup, summ, offer, curr):
        ...
        equity = summ * rates[curr] * offer.total_equity / offer.target
        balance = InvestorWallet.objects.get(investor=self, curr=curr).balance
        investable = offer.available >= equity and balance >= summ and summ >= offer.mis
    ...
    def save(self, *args, **kwargs):
        ...
        a = InvestorWallet.objects.filter(curr = 'AZN', iban_no = 'AZ9287265392706420', investor = self)
        b = InvestorWallet.objects.filter(curr = 'USD', iban_no = 'AZ9287265392706420', investor = self)
        c = InvestorWallet.objects.filter(curr = 'EUR', iban_no = 'AZ9287265392706420', investor = self)
